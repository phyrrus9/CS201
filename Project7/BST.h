...
protected:
	...
	void inorder(BSTNode<T>*, int c);
	void postorder(BSTNode<T>*);
	virtual void visit(BSTNode<T>* p, int c)
	{
		numhops += c;
		p->el.setlength(c);
		cout << p->el;
	}
	int pnodeCount(BSTNode<T> *p);
	T * pfindNode(T &, BSTNode<T> *);
	int numhops, numitems;
public:
    BST() {
        root = 0;
    }
	...
	double avghops() { return ((double)numhops/(double)numitems); }
    void clear() {
        clear(root); 
        root = 0;
    }
	...
	int nodeCount()
	{
		return pnodeCount(root);
	}
	T * findNode(T &node)
	{
		return pfindNode(node, root);
	}
};
...
template <class T>
int BST<T>::pnodeCount(BSTNode<T> *p)
{
	return p == NULL ? 0 : pnodeCount(p->left) + pnodeCount(p->right) + 1;
}

template <class T>
T * BST<T>::pfindNode(T &n, BSTNode<T> *p)
{
	if (p == NULL) return NULL;
	T *r1 = pfindNode(n, p->left), *r2 = pfindNode(n, p->right);
...
template<class T>
void BST<T>::inorder(BSTNode<T> *p, int c) {
     if (p != 0) {
         inorder(p->left, c + 1);
         visit(p, c);
         inorder(p->right, c + 1);
